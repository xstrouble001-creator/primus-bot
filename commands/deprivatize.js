...
export default {
    name: 'deprivatize',
    aliases: ['publicize', 'unlockcmd', 'ucmd'],
    description: 'Restore public/standard access to a restricted command',
    category: 'settings',
...
`│ 🔓 𝙲𝚘𝚖𝚖𝚊𝚗𝚍     : ${prefix || '#'}deprivatize\n` +
`│ 📝 𝚁𝚎𝚚𝚞𝚒𝚛𝚎𝚍    : 𝙲𝚘𝚖𝚖𝚊𝚗𝚍 𝙽𝚊𝚖𝚎\n` +
`❖─────────────────────────────❖\n\n` +
`📌 𝚄𝚂𝙰𝙶𝙴:\n` +
`➤ ${prefix || '#'}deprivatize <command_name>\n\n` +
`💡 𝙴𝚡𝚊𝚖𝚙𝚕𝚎:\n` +
`➤ ${prefix || '#'}deprivatize ban\n` +
`➤ ${prefix || '#'}deprivatize kick\n\n` +
...
`❖──────────【 𝙲𝙾𝙼𝙼𝙰𝙽𝙳  𝚂𝚃𝙰𝚃𝚄𝚂 】──────────❖\n` +
`│ 🛡️ 𝙲𝚘𝚖𝚖𝚊𝚗𝚍     : [ ${cmdName} ]\n` +
`│ 🔓 𝙰𝚌𝚌𝚎𝚜𝚜      : 𝙿𝚞𝚋𝚕𝚒𝚌 / 𝙰𝚕𝚕 𝚄𝚜𝚎𝚛𝚜\n` +
`❖─────────────────────────────❖\n\n` +
`📢 𝚃𝚑𝚎 𝚌𝚘𝚖𝚖𝚊𝚗𝚍 *${cmdName}* 𝚒𝚜\n` +
`   𝚗𝚘𝚝 𝚌𝚞𝚛𝚛𝚎𝚗𝚝𝚕𝚢 𝚕𝚘𝚌𝚔𝚎𝚍.\n\n` +
`⚡ 𝙽𝚘 𝚌𝚑𝚊𝚗𝚐𝚎𝚜 𝚠𝚎𝚛𝚎 𝚖𝚊𝚍𝚎.\n\n` +
`└─ 𝑷𝒐𝒘𝒆𝒓𝒆𝒅 𝒃𝒚 𝑷𝒓𝒊𝒎𝒖𝒔 𝑴𝒅 ──` 
                }, { quoted: msg });
            }

            const updatedLocked = currentLocked.filter(c => c !== cmdName);
            ...
            config.privateCommands = updatedLocked;

            return await sock.sendMessage(from, { 
                text: `⚡ 𝙿 𝚁 𝙸 𝙼 𝚄 𝚂   𝙼 𝙳 ⚡\n\n` +
`❖──────────【 𝙲𝙾𝙼𝙼𝙰𝙽𝙳  𝚄𝙽𝙻𝙾𝙲𝙺𝙴𝙳 】──────────❖\n` +
`│ 🔓 𝙲𝚘𝚖𝚖𝚊𝚗𝚍     : [ ${cmdName} ]\n` +
`│ 🌐 𝙰𝚌𝚌𝚎𝚜𝚜      : 𝚁𝚎𝚜𝚝𝚘𝚛𝚎𝚍 𝚏𝚘𝚛 𝙰𝚕𝚕\n` +
`│ 📁 𝙵𝚒𝚕𝚎        : 𝚄𝚙𝚍𝚊𝚝𝚎𝚍 𝚌𝚘𝚗𝚏𝚒𝚐.𝚓𝚜\n` +
`❖─────────────────────────────❖\n\n` +
`📢 𝚃𝚑𝚎 𝚌𝚘𝚖𝚖𝚊𝚗𝚍 *${cmdName}* 𝚑𝚊𝚜 𝚋𝚎𝚎𝚗\n` +
`   𝚞𝚗𝚕𝚘𝚌𝚔𝚎𝚍 𝚒𝚗 𝚌𝚘𝚗𝚏𝚒𝚐.𝚓𝚜 𝚜𝚞𝚌𝚌𝚎𝚜𝚜𝚏𝚞𝚕𝚕𝚢.\n\n` +
`└─ 𝑷𝒐𝒘𝒆𝒓𝒆𝒅 𝒃𝒚 𝑷𝒓𝒊𝒎𝒖𝒔 𝑴𝒅 ──` 
            }, { quoted: msg });

        } catch (error) {
            console.error('❌ [DEPRIVATIZE ERROR]:', error);
            return await sock.sendMessage(from, { 
